        ...
        <div className="flex justify-between">
          {FOOTER_LINKS.map((links, index) => (
            <div key={index} className="space-y-6 flex flex-col">
              <h4 className="font-gambetta text-2xl text-footer-links">({links.category})</h4>
              <div className="flex flex-col gap-4">
                {links?.links?.map((link, index) => (
                  <Link href={link.href} key={index} className="text-background text-xl leading-[130%] font-semibold">
                    {link.name}
                  </Link>
                ))}
              </div>
